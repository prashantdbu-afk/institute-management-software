...
describe("Homework Phase 2F guardrails",()=>{
 ...
 it("lets teachers resolve only batches for active course assignments",()=>{expect(sql).toContain("drop policy if exists batches_select_scoped");expect(sql).toContain("a.teacher_id=auth.uid() and a.course_id=batches.course_id and a.branch_id=batches.branch_id and a.status='active'")})
