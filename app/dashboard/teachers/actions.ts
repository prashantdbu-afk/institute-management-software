...
export async function provisionTeacherAction(input:unknown):Promise<Result>{
  ...
  try { const admin=createAdminClient(); const {data,error}=await admin.auth.admin.inviteUserByEmail(parsed.data.email,{data:{first_name:parsed.data.fullName}}); if(error||!data.user)throw error??new Error("Invitation failed"); createdId=data.user.id
    ...
    revalidatePath("/dashboard/teachers"); revalidatePath("/dashboard/users"); return{ok:true,teacher:await mappedTeacher(createdId),invitationSent:true}
